package restaurantM4N2;

import java.util.LinkedHashMap;
import java.util.Scanner;
import java.util.Map.Entry;

public class RestaurantM4N2 {
	...
	*/
	...
	Modifica la revisió de la comanda de la següent manera: si un usuari introdueix un plat que no existeix, inmediatament 
	executa una excepció general (new Exception) per avisar que el producte no existeix, per tant no s'ha d'afegir a la llista, 
	però el bucle ha de seguir executant-se.  
	Llança una excepció general (new Exception) a la pregunta Si/No per seguir demanant, en cas de que l'usuari no introdueixi 
	cap de les paraules clau. S'ha de tornar a preguntar si es vol seguir demanant o no.
	*/
	public static void demanarCom(LinkedHashMap<Integer, Integer> comanda)  throws Exception{
		...
		do {
			try{
				System.out.println("\nQuè vols demanar? Introdueix el número del plat (0-9).");
			...
			try{
				...
				if(checkDemanar.equals("s")) {
					...
					throw new Exception("\nOrdre invàlida. Has d'introduir S(si) o N(no).");
				}
			} catch (Exception e) {
				System.out.println(e.getMessage());
			}
